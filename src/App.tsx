...
import { Briefcase, FileText, Database, Server, Smartphone, Camera, Loader2, ArrowRight, ShieldCheck, Cpu, UploadCloud, MapPin, HardDrive, RefreshCw, Zap } from 'lucide-react';
...
export default function App() {
    ...
    const revealArchitecture = () => {
        ...
        setStatus('loading');
        toast.info('Decrypting Architecture Schematics...', { icon: <Loader2 className="animate-spin text-violet-400" /> });
        setTimeout(() => {
            setStatus('success');
            toast.success('Architecture Schematics Successfully Revealed');
        }, 2000);
    };
        ...
        <div className="min-h-screen p-6 md:p-12 lg:p-24 flex flex-col items-center justify-start relative overflow-x-hidden bg-slate-950 text-slate-50">
            ...
            <div className="max-w-6xl w-full z-10">
                ...
                <div className="mb-16 flex justify-center">
                    <button
                        onClick={revealArchitecture}
                        disabled={status === 'loading'}
                        className={`px-8 py-4 rounded-xl font-bold tracking-wide transition-all duration-300 flex items-center gap-3 ${
                            status === 'success' 
                            ? 'bg-slate-800 hover:bg-slate-700 text-white border border-slate-700'
                            : 'bg-gradient-to-r from-violet-600 to-emerald-600 hover:from-violet-500 hover:to-emerald-500 text-white shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:shadow-[0_0_30px_rgba(16,185,129,0.5)]'
                        } disabled:opacity-50`}
                    >
                        {status === 'loading' ? (
                            <><Loader2 className="animate-spin" size={20} /> Decrypting Nodes...</>
                        ) : status === 'success' ? (
                            'Hide Architecture Schematics'
                        ) : (
                            'Reveal Architecture Schematics'
                        )}
                    </button>
                </div>

                <div className="w-full mb-16 min-h-[400px] transition-all duration-500">
                    {status === 'idle' && (
                        <div className="glass-card p-12 text-center border border-slate-800/50 bg-slate-900/40 opacity-80 backdrop-blur-md rounded-3xl">
                             <ShieldCheck className="w-16 h-16 text-slate-500 mx-auto mb-6 opacity-50" />
                             <h3 className="text-2xl font-bold text-slate-300 mb-4">Awaiting Authorization</h3>
                             <p className="text-slate-500 max-w-md mx-auto">
                                Click the button above to initialize the decryption sequence and view the architectural blueprints for the Insurance AI and Enterprise Photo platforms.
                             </p>
                        </div>
                    )}

                    {status === 'loading' && (
                        <div className="glass-card p-12 text-center border border-violet-500/20 bg-slate-900/60 rounded-3xl flex flex-col items-center justify-center min-h-[300px]">
                            <div className="relative w-24 h-24 mb-8">
                                ...
                                <Cpu className="absolute inset-0 m-auto text-violet-400 w-8 h-8 animate-pulse" />
                            </div>
                            <h3 className="text-xl font-bold text-violet-300 animate-pulse">Compiling Node Topologies...</h3>
                        </div>
                    )}
                            ...
                            <div className="glass-card p-8 md:p-12 relative overflow-hidden border-violet-500/30 bg-slate-900/80 rounded-3xl group transition-all hover:border-violet-500/50 hover:shadow-[0_0_40px_rgba(139,92,246,0.1)]">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12">
                                    ...
                                    <div className="bg-slate-950/60 p-4 rounded-xl border border-slate-800">
                                        ...
                                        <div className="text-white font-medium">Google Sheets & Drive</div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-950/80 rounded-2xl p-6 border border-slate-800/80">
                                    ...
                                    <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
                                        ...
                                        <div className="flex flex-col items-center text-center p-4 bg-blue-900/20 rounded-xl border border-blue-500/30 w-full lg:w-1/5 shadow-[0_0_15px_rgba(59,130,246,0.1)]">
                                            ...
                                            <div className="text-sm font-bold text-white mb-1">Sheets & Drive</div>
                                            <div className="text-xs text-slate-500">Log & Store PDF</div>
                                        </div>
